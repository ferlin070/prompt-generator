...
async function upgradePlan(planId, billingCycle='monthly', promoCode='') {
  ...
  if (promoCode) {
    try {
      const data = await api(`/api/util?type=validate-promo&code=${encodeURIComponent(promoCode)}`);
      if (data.valid) discount = data.code.type==='percentage' ? price*(data.code.value/100) : data.code.value;
    } catch {}
  }
  ...
  const updateRes = await updateUserProfile({
    plan: planId, plan_billing_cycle: billingCycle, plan_started_at: new Date().toISOString(),
    plan_expires_at: expiresAt, ai_credits_left: plan.limits.aiCalls===-1 ? 99999 : plan.limits.aiCalls,
    is_trial: true, trial_ends_at: new Date(Date.now()+7*864e5).toISOString(),
  });
  if (!updateRes.success) return { success:false, message: updateRes.message };

  await api('/api/util', { method: 'POST', body: JSON.stringify({
    type: 'subscription', userId: session.userId, plan: planId, billing_cycle: billingCycle,
...
async function checkLimit(feature) {
  ...
  switch(feature) {
    case 'createPrompt':
      ...
      if ((stats.thisMonth||0) >= plan.limits.promptsPerMonth) return {allowed:false, reason:`Had bulanan (${plan.limits.promptsPerMonth}) dicapai.`, upgrade:true};
    ...
    case 'savePrompt':
      ...
      if ((stats.total||0) >= plan.limits.savedPrompts) return {allowed:false, reason:`Had simpanan (${plan.limits.savedPrompts}) dicapai.`, upgrade:true};
      return {allowed:true};
    case 'aiGenerate':
...
async function deductAICredit(amount=1) {
  ...
  await updateUserProfile({ ai_credits_left: Math.max(0, (profile?.ai_credits_left||0) - amount) });
}
...
function showUpgradePrompt(reason='') {
  ...
  if (!modal) {
    ...
    modal.innerHTML = `<div class="modal" style="max-width:460px;text-align:center"><div class="modal-header"><h3 class="modal-title"><i class="fa fa-rocket"></i> Naik Taraf Pelan</h3><button class="modal-close" onclick="closeModal('upgradeModal')"><i class="fa fa-xmark"></i></button></div><div class="modal-body"><div style="font-size:2.5rem;margin-bottom:12px">${next.icon}</div><p id="upgradeReason" style="color:var(--text-secondary);margin-bottom:20px"></p><div style="background:var(--bg-primary);border:1px solid var(--border);border-radius:10px;padding:20px;margin-bottom:20px"><div style="font-size:1.6rem;font-weight:800;color:${next.color}">RM${next.price.monthly}<span style="font-size:.9rem;color:var(--text-muted)">/bulan</span></div><ul style="list-style:none;text-align:left;font-size:.85rem;color:var(--text-secondary);display:flex;flex-direction:column;gap:6px;margin-top:12px">${next.features.slice(0,4).map(f=>`<li><i class="fa fa-check"></i> ${f}</li>`).join('')}</ul></div><a href="pricing.html" class="btn btn-primary btn-block" style="margin-bottom:10px">${next.cta}</a><button class="btn btn-ghost btn-block btn-sm" onclick="closeModal('upgradeModal')">Mungkin lain kali</button></div></div>`;
...
async function validatePromoCode(code) {
  try { return await api(`/api/util?type=validate-promo&code=${encodeURIComponent(code)}`); } catch { return {valid:false, message:'Kod promo tidak sah.'}; }
}
